...
| 29 | **Descending floors** — collapse after two more crossings | 085, 086, 087 | **done** — a square drops one step each time the ball rolls onto it anew, 085 to 086 to 087 to an open pit, so it takes three crossings. Still missing: the original's other way of spelling this, a plain floor marked vanishing by `0x20` in its attribute byte, which spends its first crossing becoming 085 and so takes **four**. It needs a per-square attribute the level format here does not carry yet, and must not be mapped onto 085 — that would be a crossing short |
